...
arr1=[]
arr2=[]
def print_diff(c, x, y,str1,str2, i, j):
    """Print the diff using LCS length matrix by backtracking it"""

    if i < 0 and j < 0:
        return ""
    elif i < 0 and j >=0:
        print_diff(c, x, y,str1,str2, i, j - 1)
        print("+ " + str2[j].strip("\n"))
    elif j < 0 and i >=0:
        print_diff(c, x, y,str1,str2, i - 1, j)
        print("- " + str1[i].strip("\n"))
    elif x[i] == y[j]:
        print_diff(c, x, y,str1,str2, i - 1, j - 1)
        print("  " + str1[i].strip("\n"))
...
def diff(x, y,str1,str2):
    ...
    return print_diff(c,x, y, str1, str2, len(x)-1, len(y)-1)
