...
def MultiplePlots(xValues, allYValues):
    ...
    for yValues in allYValues:
        plt.plot(list(map(float, xValues)), list( map(float, yValues) ))
...
def GetXandYValues(coordinates):
    ...
    for file in coordinates:
        ...
        yValues = []
        for coordinate in file:
            yValues.append(coordinate[2])

        allYValues.append(yValues)
        yValues = []
        for coordinate in file:
            yValues.append(coordinate[3])
        allYValues.append(yValues)
        yValues = []
...
print(str(allYValues)+"\n")
print(xValues)
print(GetCoordinates( numFiles, separator , fExtension))
